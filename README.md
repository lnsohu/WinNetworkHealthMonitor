...
What was added (MVP):
- `netlify/functions/health-report.js` — POST endpoint to receive kiosk reports. Requires `x-api-key` header matching `API_KEY` environment variable.
- `netlify/functions/get-status.js` — GET endpoint to return the most recently received status per kiosk (reads in-memory store).
- `web/index.html` — Minimal static dashboard that polls `/.netlify/functions/get-status` and shows basic columns.
- `netlify.toml` — Netlify config (functions + publish dir).
